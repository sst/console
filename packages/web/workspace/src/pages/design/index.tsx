...
function generateBarHeights(
  ...
  const newData = data.map(({ value }, i) => {
    ...
    return { value, height: roundedHeight, index: i };
  });
...
function Histogram(props: HistogramProps) {
  ...
  const barData = generateBarHeights(props.height, props.data);
  const [showTooltip, setShowTooltip] = createSignal<number | null>(null);
